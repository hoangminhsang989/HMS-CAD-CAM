...
class MaterialStateStore:
    ...
    def write(self, project_root: Path, state: MaterialState) -> Path:
        if state.status is not MaterialStateStatus.COMPLETE or not state.content_is_verified:
            raise CamValidationError("Only verified COMPLETE material states may be published")
    ...
    def load(self, project_root: Path, fingerprint: ContentFingerprint) -> MaterialStateLoad:
        try:
            if not isinstance(fingerprint, ContentFingerprint):
                return MaterialStateLoad(MaterialStateLoadStatus.INCOMPATIBLE, message="state fingerprint is invalid")
            ...
            state = material_state_from_persisted_bytes(payload, fingerprint)
