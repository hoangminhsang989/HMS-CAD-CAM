...
from hms_cadcam.cam.domain.rest_finishing import (
    REST_FINISHING_PARAMETER_SCHEMA_VERSION, REST_FINISHING_STRATEGY_KEY,
    REST_FINISHING_STRATEGY_VERSION, RestFinishingParameters,
...
class RegisteredOperationType:
    ...
    def __post_init__(self) -> None:
        known = {
            ...
            (REST_FINISHING_STRATEGY_KEY, REST_FINISHING_STRATEGY_VERSION,
             REST_FINISHING_PARAMETER_SCHEMA_VERSION),
        }
        if (self.strategy_key, self.strategy_version, self.parameter_schema_version) not in known:
            raise CamValidationError("Unsupported registered operation type")


class RestContourOperationRegistry:
    """Discover, create and validate the registered Rest operation types."""
    ...
    def register_default(self) -> "RestContourOperationRegistry":
        ...
        finishing = RegisteredOperationType(
            REST_FINISHING_STRATEGY_KEY, REST_FINISHING_STRATEGY_VERSION,
            REST_FINISHING_PARAMETER_SCHEMA_VERSION,
        )
        self._registered[(finishing.strategy_key, finishing.strategy_version,
                          finishing.parameter_schema_version)] = finishing
        return self
    ...
    def create_for_strategy(self, strategy_key: str, **kwargs: Any) -> Operation:
        ...
        if strategy_key == REST_FINISHING_STRATEGY_KEY:
            return self.create_rest_finishing(**kwargs)
    ...
    def create_rest_finishing(
        self, *, operation_id: OperationId, node_id: CamNodeId, setup_id: SetupId,
        parameters: RestFinishingParameters, profile: RestFinishingProfileSelection,
        dependency_operation_id: OperationId, tool_assembly: ToolAssembly,
        machine_requirement: MachineRequirement,
    ) -> Operation:
        """Create the manual-only v1 Rest Finishing aggregate record."""
        if not self.is_registered(REST_FINISHING_STRATEGY_KEY):
            raise CamValidationError("Rest Finishing operation type is not registered")
        if not isinstance(operation_id, OperationId) or not isinstance(node_id, CamNodeId) or not isinstance(setup_id, SetupId):
            raise CamValidationError("Rest Finishing operation identity is invalid")
        if not isinstance(parameters, RestFinishingParameters) or not isinstance(profile, RestFinishingProfileSelection):
            raise CamValidationError("Rest Finishing parameters or profile are invalid")
        profile.validate_for(parameters)
        expected_profile_kind = _rest_contour_profile_kind(parameters.profile_source)
        if profile.descriptor.reference.kind is not expected_profile_kind:
            raise CamValidationError("Rest Finishing profile source and reference kind disagree")
        if not isinstance(dependency_operation_id, OperationId) or dependency_operation_id == operation_id:
            raise CamValidationError("Rest Finishing material-state dependency is invalid")
        if not isinstance(tool_assembly, ToolAssembly) or not isinstance(machine_requirement, MachineRequirement):
            raise CamValidationError("Rest Finishing tool or machine requirement is invalid")
        if parameters.unit is not profile.descriptor.unit or parameters.unit is not tool_assembly.unit or machine_requirement.unit is not parameters.unit:
            raise CamValidationError("Rest Finishing units are inconsistent")
        if OperationCapability.MILLING not in machine_requirement.required_capabilities:
            raise CamValidationError("Rest Finishing machine must require milling capability")
        return Operation(
            operation_id, node_id, OperationFamily.MILLING, setup_id,
            ToolAssemblyReference.from_assembly(tool_assembly),
            (OperationGeometryInput(GeometryInputId.new(), GeometryInputRole.PROFILE,
                profile.descriptor.reference, True, expected_kind=expected_profile_kind,
                selection_order=0),), parameters.to_operation_parameters(), machine_requirement,
        )
    ...
    def decode(self, payload: Mapping[str, Any]) -> Operation:
        ...
        if operation.strategy_key not in {REST_CONTOUR_STRATEGY_KEY, REST_FINISHING_STRATEGY_KEY}:
            return operation
        if not self.is_registered(
            operation.strategy_key, operation.strategy_version,
            operation.parameters.schema_version,
        ):
            raise UnsupportedCamSchemaError("Unsupported registered Rest operation version")
        if operation.strategy_key == REST_FINISHING_STRATEGY_KEY:
            ...
            expected_profile_kind = _rest_contour_profile_kind(parameters.profile_source)
            if (operation.family is not OperationFamily.MILLING or len(operation.geometry_inputs) != 1
                    or len(profiles) != 1 or not profiles[0].required
                ...
                raise CamValidationError("Persisted Rest Finishing operation contract is invalid")
            return operation
