...
class PostProcessorStudioPanel(QWidget):
    ...
    def _build(self) -> None:
        ...
        self.search = QLineEdit(); self.search.setPlaceholderText("Tìm Post / revision")
        ...
        self.new_button.clicked.connect(lambda: self.message.emit("Tạo Post yêu cầu metadata và source hợp lệ."))
        self.import_button.clicked.connect(lambda: self.message.emit("Nhập Post chỉ tạo revision immutable trong workspace cô lập."))
    ...
    def _show_selected(self) -> None:
        ...
        entries = (("Revision", revision.revision_id), ("SHA-256", revision.source_sha256), ("Status", self._service.lifecycle_status(revision_id).value), ("Encoding", revision.source_encoding), ("Line ending", revision.line_ending), ("Deployment", "NOT_ACTIVE_GLOBALLY"))
