<#
.SYNOPSIS
Run the exact R239 full HMS suite with file-backed terminal evidence.
#>
...
$baseTemp = Join-Path $projectRoot '.pytest_tmp\r239_full_suite_final'
[IO.Directory]::CreateDirectory((Split-Path -Parent $baseTemp)) | Out-Null
...
try {
    & $pythonPath -m pytest -p pytestqt.plugin -o qt_api=pyside6 `
        '--basetemp=.pytest_tmp/r239_full_suite_final' `
...
[ordered]@{
    format = 'HMS_R239_FILE_BACKED_FULL_SUITE'
    format_version = 1
    command = @($pythonPath, '-m', 'pytest', '-p', 'pytestqt.plugin', '-o', 'qt_api=pyside6', '--basetemp=.pytest_tmp/r239_full_suite_final', '-m', 'not benchmark and not windows_native', "--junitxml=$junit")
