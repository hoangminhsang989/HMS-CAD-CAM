...
- Chương trình được chọn: `STAGE16A_3STEP_CAM_OPERATION_WORKFLOW_AND_PRODUCTION_TOOL_LIBRARY_FOUNDATION`.
...
Luồng ownership production hiện tại là:

`ProjectSession → CamProjectSnapshot → CamJob → Setup → OperationTree → Operation`
...
- Dependency change: **0**.
- CAM algorithm, collision/contact solver, Simulation, Post change: **0**.
